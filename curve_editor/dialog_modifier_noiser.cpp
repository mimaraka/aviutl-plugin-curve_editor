...
namespace cved {
	int NoiserModifierDialog::i_resource() const noexcept { return IDD_MODIFIER_NOISER; }


	void NoiserModifierDialog::init_controls(HWND hwnd, const Noiser* p_noiser) noexcept {
		...
		::SetWindowTextA(hwnd_edit_seed_, std::to_string(p_noiser->seed()).c_str());
		...
		::SendMessageA(hwnd_slider_phase_, TBM_SETRANGE, TRUE, MAKELPARAM(0, 20000));
		...
		::SendMessageA(hwnd_slider_amplitude_, TBM_SETPOS, TRUE, static_cast<LPARAM>(p_noiser->amplitude() * 100));
		::SendMessageA(hwnd_slider_frequency_, TBM_SETPOS, TRUE, static_cast<LPARAM>(p_noiser->frequency() * 10));
		::SendMessageA(hwnd_slider_phase_, TBM_SETPOS, TRUE, static_cast<LPARAM>(p_noiser->phase() * 100 + 10000));
		::SendMessageA(hwnd_slider_octaves_, TBM_SETPOS, TRUE, static_cast<LPARAM>(p_noiser->octaves()));
		::SendMessageA(hwnd_slider_decay_sharpness_, TBM_SETPOS, TRUE, static_cast<LPARAM>(p_noiser->decay_sharpness() * 10));
	...
	INT_PTR NoiserModifierDialog::dialog_proc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) {
		static Noiser* p_noiser = nullptr;
		static Noiser noiser_prev;

		switch (message) {
		case WM_INITDIALOG:
			p_noiser = reinterpret_cast<Noiser*>(lparam);
			if (!p_noiser) {
				::EndDialog(hwnd, IDCANCEL);
				return TRUE;
			}
			noiser_prev = *p_noiser;
			init_controls(hwnd, p_noiser);
			return TRUE;
		...
		case WM_HSCROLL:
			if (lparam == (LPARAM)hwnd_slider_amplitude_) {
				...
				double value = static_cast<double>(::SendMessageA(hwnd_slider_phase_, TBM_GETPOS, NULL, NULL) - 10000) * 0.01;
				if (p_noiser) {
					p_noiser->set_phase(value);
				}
				...
				if (p_noiser) {
					p_noiser->set_decay_sharpness(value);
				}
