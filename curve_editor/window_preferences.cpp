...
namespace curve_editor {
	LRESULT CALLBACK PreferencesWindow::wndproc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) {
		...
		case WM_CREATE:
			...
			global::webview.set(WebViewType::Preferences, my_webview);
			global::webview.switch_to(WebViewType::Preferences);
			return 0;
		...
		case WM_DESTROY:
		{
			...
			global::config.set_preferences_window_size(mkaul::Size{ static_cast<uint32_t>(rect.right), static_cast<uint32_t>(rect.bottom) });
			my_webview.destroy();
		...
		case WM_COMMAND:
			switch (wparam) {
			case (WPARAM)WindowCommand::PreferencesClose:
				::DestroyWindow(hwnd);
				break;
			}
			return 0;
	...
	HWND PreferencesWindow::create(HWND hwnd) noexcept {
		using StringId = global::StringTable::StringId;

		hwnd_parent_ = hwnd;
		auto window_size = global::config.get_preferences_window_size();
		...
		return mkaul::ui::Window::create(
			...
			hwnd,
			global::string_table[StringId::CaptionPreferences],
			L"PreferencesWindow",
			&wndproc,
