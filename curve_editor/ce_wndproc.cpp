...
LRESULT CALLBACK wndproc_graph(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	case WM_LBUTTONDOWN:
		...
		if (!g_config.mode) {
			// 制御点2がクリックされたとき
			if (g_curve_value.point_in_ctpts(cl_pt) == 2) {
				move_pt = 2;
				pt_trace[0] = g_curve_value.ctpt[0];
				pt_trace[1] = g_curve_value.ctpt[1];
			...
			// 制御点1がクリックされたとき
			else if (g_curve_value.point_in_ctpts(cl_pt) == 1) {
				move_pt = 1;
				pt_trace[0] = g_curve_value.ctpt[0];
				pt_trace[1] = g_curve_value.ctpt[1];
				SetCursor(LoadCursor(NULL, IDC_HAND));
		...
		// 値モードでないとき
			...
			// カーソルが制御点上にあるかどうか
			address = g_curve_id[g_config.id_current].pt_in_ctpt(cl_pt);
	...
	case WM_MOUSEMOVE:
		...
		//IDモード
			...
			if (g_curve_id[g_config.id_current].pt_in_ctpt(cl_pt).position > 0)
				SetCursor(LoadCursor(NULL, IDC_HAND));
		}
