...
## Q&A
**Q.** このスクリプトを他のディレクトリのAviUtlに移動させたら設定とかプリセットが消えました

**A.** Preferencesの設定内容やカスタムテーマの情報は「aviutl.ini」内に，カーブのプリセット情報は「curve_editor.bin」内に保存されています。設定情報をコピーしたい場合は，aviutl.ini内から「[Curve Editor]」という名前のセクションを頑張って探し，その下にある情報を移動先のaviutl.iniにコピーします。プリセットをコピーしたい場合は，「curve_editor.bin」をそのままコピーすれば大丈夫です。  
<br/>  
**Q.** このプラグインだけダークテーマなのがミスマッチなんですけど  

**A.** カーブエディタ上で右クリック→Preferences(設定)→Theme(テーマ)から他のテーマを選択することができます。デフォルトでライトテーマを用意していますが，気に入らない場合はCustom(カスタム)を選択し横にあるボタンをクリックしてオリジナルのテーマを作成することもできます。  
